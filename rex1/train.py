...
def main() -> None:
    ...
    train_loader, val_loader = build_dataloaders(data_cfg, train_cfg)
    ...
    start_step = load_checkpoint(path=args.resume or train_cfg.get("resume"), model=model, optimizer=optimizer, device=device)
    ...
    wandb_run = init_wandb(cfg, getattr(model, "_orig_mod", model), train_cfg)

    print(
        f"training on {device}, starting at step {start_step}, "
        f"epochs={epochs}, steps_per_epoch={steps_per_epoch}, target_steps={max_steps}"
    )
    model.train()
    step = start_step
    tokens_per_step = int(data_cfg.get("block_size", rex_cfg.max_seq_len)) * int(train_cfg.get("batch_size", 1)) * grad_accum
    pbar = tqdm(total=max_steps, initial=min(start_step, max_steps), desc="train")
    last_log = time.time()
